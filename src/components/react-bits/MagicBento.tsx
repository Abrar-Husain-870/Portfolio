...
const DEFAULT_GLOW_COLOR = '80, 140, 255'
const MOBILE_BREAKPOINT = 768

type BentoItem = { color?: string; title: string; description: string; label?: string; image?: string; large?: boolean; repo?: string; demo?: string }

const cardData: BentoItem[] = [
...
const updateCardGlowProperties = (
  ...
  ;(card.style as CSSStyleDeclaration).setProperty('--glow-x', `${relativeX}%`)
  ;(card.style as CSSStyleDeclaration).setProperty('--glow-y', `${relativeY}%`)
  ;(card.style as CSSStyleDeclaration).setProperty('--glow-intensity', glow.toString())
  ;(card.style as CSSStyleDeclaration).setProperty('--glow-radius', `${radius}px`)
}
...
type ParticleCardProps = {
  ...
  style?: CSSProperties & { ['--glow-color']?: string }
...
const GlobalSpotlight = ({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const targetOpacity = minDistance <= prox ? 0.8 : minDistance <= fade ? ((fade - minDistance) / (fade - prox)) * 0.8 : 0
...
export default function MagicBento({
  ...
  const data: BentoItem[] = (items && items.length ? items : cardData).map((c) => ({
    color: c.color ?? '#060010',
    title: c.title,
    description: c.description,
    label: c.label ?? 'Project',
    image: c.image,
    large: c.large ?? false,
    repo: (c as any).repo,
    demo: (c as any).demo,
  ...
  return (
    <>
      ...
      <BentoCardGrid gridRef={gridRef}>
        {data.map((card, index) => {
          const baseClassName = `card ${textAutoHide ? 'card--text-autohide' : ''} ${enableBorderGlow ? 'card--border-glow' : ''} ${card.large ? 'card--large' : ''}`
          const cardProps = {
            className: baseClassName,
            style: { background: 'var(--background-dark)', '--glow-color': glowColor } as CSSProperties & { ['--glow-color']?: string },
          }
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      {card.demo && (
                        <a
                          ...
                          className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 text-white ring-1 ring-white/20 transition"
                      ...
                      {card.repo && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-white/5 hover:bg-white/10 text-white ring-1 ring-white/20 transition"
